...
> **Shipped:** The KDF primitive (`crypto/kdf.rs`, versioned `KdfParams`, Argon2id `m=64MiB, t=3, p=4` default with a PBKDF2-SHA512 fallback for legacy reads) landed first, then the live wiring (#252): setup/unlock derive the master with Argon2id from a plaintext `vault.kdf.json` sidecar (salt+params, read before the DB is opened — the descriptor can't live in encrypted `meta`, which isn't readable until the key exists), HKDF-split into the SQLCipher key + a per-entry AES-256-GCM payload key. `change_master_password` re-derives (fresh salt), re-seals every payload, rekeys the DB, and rewrites the sidecar atomically with a snapshot-based recovery point. Legacy PBKDF2 `Cryptor` + golden/crosscheck harness untouched.
