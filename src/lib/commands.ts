 ...
 */
export const shareOpen = (link: string): Promise<Entry> =>
  invoke('share_open', { link })

// Delete the uploaded file, before its 24 hours are up.
export const shareRevoke = (fileId: string): Promise<void> =>
  invoke('share_revoke', { fileId })

export const shareList = (): Promise<ActiveShare[]> => invoke('share_list')
