...
export default function Tooltip({ content, children }: Props) {
  return (
    <div className="group/tt relative">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-full z-50 mx-auto mt-2 w-max whitespace-nowrap rounded-sm bg-text px-2.5 py-1 text-base text-detail opacity-0 shadow-[var(--shadow)] transition-opacity delay-0 duration-150 group-hover/tt:animate-pop group-hover/tt:opacity-100 group-hover/tt:delay-300 group-hover/tt:[animation-delay:300ms] group-focus-within/tt:animate-pop group-focus-within/tt:opacity-100 group-focus-within/tt:delay-300 group-focus-within/tt:[animation-delay:300ms]">
        {content}
      </div>
      {children}
    </div>
