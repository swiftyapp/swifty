...
interface Props {
  /**
   * An unlock is being verified or held on its success beat. The chip stays
   * as it is but opens nothing, and the chords stand down: the backend is
   * opening *this* vault, and a switch landing now would have that unlock
   * enter with its rows after the backend had moved to another one.
   */
  busy?: boolean
}
...
// choice to make.
export default function WorkspacePicker({ busy = false }: Props) {
  ...
  useWorkspaceShortcuts(list.length < 2 || busy ? [] : list, id =>
    open ? pick(id) : select(id)
  )
  ...
  return (
    <div data-testid="workspace-picker" className="relative mb-5 flex justify-center">
      <Chip
        ...
        workspace={current}
        open={open}
        onClick={() => setOpen(value => !value && !busy)}
      />
      {open && (
