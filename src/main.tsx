...
// screen, which is never what the keystroke was meant to do here. Read off the
// physical key, so the block holds on a layout that prints something other
// than `r` there.
const blockReloadShortcuts = () => {
  window.addEventListener('keydown', e => {
    const code = keyCode(e)
    const isReload = code === 'F5' || ((e.metaKey || e.ctrlKey) && code === 'KeyR')
