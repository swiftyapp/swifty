...
| Favorites and Archive views in the rail | item 14 | `view-favorites` / `view-archive`, in rail order All Items · Favorites · Archive · Tags · spacer · `lock-vault-button` · Settings (the generator has no tile: `generator-button` is a row in the Add picker, and ⌘G / the palette open the same dialog). Each view retitles `list-title` ("Favorites", "Archive"); search and the kind chips apply in all three item views. |
